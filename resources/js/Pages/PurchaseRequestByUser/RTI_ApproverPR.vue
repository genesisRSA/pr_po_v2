        ...
        <v-row>
              ...
              <v-divider></v-divider>
                    ...
                    >
                        ...
                        <template v-slot:item.actions="{ item }">
                            ...
                            <div v-if='item.status=="PENDING RTI APPROVER" || item.status=="PR DECLINED BY RTI APPROVER"'>
                                <v-tooltip bottom>
                                <template v-slot:activator="{ on, attrs }">
                                    <v-icon
                                        small
                                ...
                                <template v-slot:activator="{ on, attrs }">
                                    <v-icon
                                        small
                                        class="mr-2"
                                        @click="declinePresidentialPR(item)"
                                        v-bind="attrs"
                                        v-on="on"
                                    >
                                       mdi-thumb-down
          ...
          <v-btn
            ...
            <v-dialog v-model="dialogDeletePR" max-width="500px">
              ...
              <v-dialog v-model="approvePresidentDialog" max-width="500px">
                            <v-card>
                                ...
                                <v-card-actions>
                                <v-spacer></v-spacer>
                                <v-btn color="blue darken-1" text @click="closeApprovePresidentDialog()">Cancel</v-btn>
                                <v-btn color="blue darken-1" text @click="ApprovePR()">OK</v-btn>
                                <v-spacer></v-spacer>
                                </v-card-actions>
                            </v-card>
              </v-dialog>
              <v-dialog v-model="declinePresidentDialog" max-width="500px">
                            <v-card>
                                <v-card-title class="justify-center approve-text"><h5>Have you already browse the PR details? <br>Will you <span style="color : red !important;">DECLINE</span> this PR?</h5></v-card-title>
                                <v-card-actions>
                                <v-spacer></v-spacer>
                                <v-btn color="blue darken-1" text @click="closeDeclinePresidentDialog()">Cancel</v-btn>
                                <v-btn color="blue darken-1" text @click="DeclinePR()">OK</v-btn>
                                <v-spacer></v-spacer>
                                </v-card-actions>
                            </v-card>
              </v-dialog>
...
<script>
    ...
    methods: {
        ...
        approvePresidentialPR(item){
            this.approvePresidentDialog = true
            this.selectedForPRChanges = item
        },

        closeApprovePresidentDialog(){
            this.approvePresidentDialog = false
        },
        ApprovePR(){

                axios.post('/ApprovePRRTIApprover', this.selectedForPRChanges )
                .then(response =>{
        ...
        declinePresidentialPR(item){
            this.declinePresidentDialog = true
            this.selectedForPRChanges = item
        },

        closeDeclinePresidentDialog(){
            this.declinePresidentDialog = false
        },

        DeclinePR(){
                axios.post('/DeclinePRRTIApprover', this.selectedForPRChanges )
                .then(response =>{
                    console.log(response.data)
                        this.closeDeclinePresidentDialog()
                        this.getMyPRlist()
                })
                .catch(error =>{
                        console.log(error.response);
